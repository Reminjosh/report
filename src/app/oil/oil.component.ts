...
export class OilComponent implements OnInit {
  ...
  kgsCalculate() {
    ...
    const oilValue = localStorage.getItem("oilGram")
    const remainingTotal =  Number(startCount) - (Number(studentsCount) * Number(days) * Number(oilValue)) + Number(credit);
    this.budgetCalculate.controls["remainingKgs"].setValue((remainingTotal).toFixed(2) + " Lt")
    const total = Number(startCount) + Number(credit);
    this.budgetCalculate.controls["totalKgs"].setValue((total).toFixed(2) + " Lt");
    const givenTotal = Number(studentsCount) * Number(days) * Number(oilValue);
    this.budgetCalculate.controls["givenKgs"].setValue((givenTotal).toFixed(2) + " Lt");
  }
