...
@Component({
  selector: 'app-dhal',
  templateUrl: './dhal.component.html',
  styleUrls: ['./dhal.component.scss']
})
export class DhalComponent implements OnInit {
  isModalShow = true;
  ...
  kgsCalculate() {
    ...
    const dhalValue = localStorage.getItem("dhalGram")
    const remainingTotal =  Number(startCount) - (Number(studentsCount) * Number(days) * Number(dhalValue)) + Number(credit);
    ...
    const givenTotal = Number(studentsCount) * Number(days) * Number(dhalValue);
