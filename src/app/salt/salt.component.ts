...
export class SaltComponent implements OnInit {
  ...
  kgsCalculate() {
    ...
    const remainingTotal =  Number(startCount) - (Number(studentsCount) * Number(days) * Number(this.saltGramsValue)) + Number(credit);
    ...
    const givenTotal = Number(studentsCount) * Number(days) * Number(this.saltGramsValue);
