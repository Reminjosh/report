...
@Component({
  selector: 'app-konda-kadalai',
  templateUrl: './konda-kadalai.component.html',
  styleUrls: ['./konda-kadalai.component.scss']
})
export class KondaKadalaiComponent implements OnInit {
  isModalShow = true;
  ...
  kgsCalculate() {
    ...
    const kadalaiValue = localStorage.getItem("koKadalaiGram")
    const remainingTotal =  Number(startCount) - (Number(studentsCount) * Number(days) * Number(kadalaiValue)) + Number(credit);
    ...
    const givenTotal = Number(studentsCount) * Number(days) * Number(kadalaiValue);
